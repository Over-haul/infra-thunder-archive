...
def create_kubernetes_rootca(
    ...
    k8s_ca = self_signed_cert.SelfSignedCert(
        ...
        key_algorithm=k8s_key.algorithm,
        validity_period_hours=TEN_YEARS_IN_HOURS,
        is_ca_certificate=True,
        subjects=[
            cert_request.SelfSignedCertSubjectArgs(
                common_name=f"Kubernetes Cluster {cluster_name} Root CA 1",
...
def create_kubernetes_proxy_rootca(
    ...
    k8s_proxy_ca = self_signed_cert.SelfSignedCert(
        ...
        key_algorithm=k8s_proxy_key.algorithm,
        validity_period_hours=TEN_YEARS_IN_HOURS,
        is_ca_certificate=True,
        subjects=[
            cert_request.SelfSignedCertSubjectArgs(
                common_name=f"Kubernetes Cluster {cluster_name} Proxy Root CA 1",
                organization="Thunder",
                organizational_unit=f"{get_sysenv()}",
                country="US",
            )
...
def create_etcd_root_ca(
    ...
    etcd_ca = self_signed_cert.SelfSignedCert(
        ...
        key_algorithm=etcd_key.algorithm,
        validity_period_hours=TEN_YEARS_IN_HOURS,
        is_ca_certificate=True,
        subjects=[
            cert_request.SelfSignedCertSubjectArgs(
                common_name=f"etcd Cluster {cluster_name} Root CA 1",
                organization="Thunder",
...
def create_admin_client_cert(
    ...
    admin_csr = cert_request.CertRequest(
        ...
        key_algorithm=admin_key.algorithm,
        subjects=[
            cert_request.CertRequestSubjectArgs(
    ...
    admin_cert = locally_signed_cert.LocallySignedCert(
        ...
        ca_key_algorithm=k8s_key.algorithm,
...
def create_service_account_keypair(cls, dependency: ComponentResource) -> private_key.PrivateKey:
